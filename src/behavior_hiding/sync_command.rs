...
impl RepositoryCommands {
        ...
        loop {
            ...
            match action.trim() {
                ...
                "6" => {
                    ...
                    let parent_commit = self.get_user_input("Enter the parent commit ID (optional): ");
                    let parent = if parent_commit.trim().is_empty() { None } else { Some(parent_commit) };
    ...
    pub fn inspect_repo(&self, options: RepoOptions, inspection_type: InspectionType) -> Result<InspectionResult, StatusError> {
    ...
    pub fn manage_revisions(&self, options: RevisionOptions, revision_action: RevisionAction) -> Result<RevisionResult, StatusError> {
        let repo_path = match &self.repo_options.path {
            Some(path) => path,
            None => return Err(StatusError::ActionFailed("Repository path not set".to_string())),
        };
        match revision_action {
            RevisionAction::Diff => {
                // Mock function call - Replace with actual Repository Hiding function
                Ok(RevisionResult::DiffResult { changes: "modified file1.txt".to_string() })
            }
            RevisionAction::Cat => {
                Ok(RevisionResult::CatResult { content: "File content from specified revision".to_string() })
            }
            RevisionAction::Checkout => {
                ...
                if let Some(ref_name) = options.revision_id.clone() {
                    ...
                    Ok(RevisionResult::CheckoutResult { success_message: format!("Checked out revision {}", ref_name) })
                } else {
            ...
            RevisionAction::Commit => {
                println!("Committing all changes in repository at path: {}", repo_path);
                ...
                let commit = new_commit(
                    commit_id.clone(),
                    options.commit_message.unwrap_or("Default commit message".to_string()),
                    options.author.unwrap_or("Author Name".to_string()),
                    Utc::now().to_rfc3339(),
                    options.parent_commit,
                );

                println!("Commit completed successfully for repository at {} with ID: {}", repo_path, commit.id);
                Ok(RevisionResult::CommitResult { commit_id: commit.id })
            }
            _ => Err(StatusError::InvalidCommand),
        
            RevisionAction::Log => {
                Ok(RevisionResult::LogResult { history: vec!["Commit 1".to_string(), "Commit 2".to_string()] })
            }
        }
    }
}
